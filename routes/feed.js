...
*/
var youtubeAPIKeys = ["AIzaSyBMti91-v5LJmH4_l4LWQTavnp8Td2WS88", 
...
*/
var newsapiAPIKeys = ["a02d4676a3394acebe3f2ca3a4d073e3",
...
var espn_info = {
    ...
    undisputed_info = {
    name: "Undisputed",
    logo: "https://b.fssta.com/sta/images/navSection/images/shows/nav_shows_undisputed.vadapt.160.high.0.png"
},
    nba_info = {
    name: "NBA.com",
            ...
            if (sport === "nba" || sport === "nfl"){
                ...
                */
                if (req.params.type.toLowerCase() === "a"){
                    ...
                    var espn = await getArticles("espn", "espn.com", teamName, newsapiAPIKeys[0]);
                    var espn_result = {name: espn_info.name, logo: espn_info.logo, content: espn.articles};
                    ...
                    var fox = await getArticles("fox-sports", "foxsports.com", teamName, newsapiAPIKeys[2]);
                    var fox_result = {name: fox_info.name, logo: fox_info.logo, content: fox.articles};
                ...
                */
                    ...
                    var br = await getVideos(teamName, channelIDs.br, "bleacherreport", youtubeAPIKeys[3]);
                    var br_result = {name: bleacher_report_info.name, logo: bleacher_report_info.logo, content: br.items};
                    results.push(br_result);
                    // Request ESPN videos from Youtube
                    var espn = await getVideos(teamName, channelIDs.espn, "espn", youtubeAPIKeys[0]);
                    var espn_result = {name: espn_info.name, logo: espn_info.logo, content: espn.items};
                    results.push(espn_result);
                    // Request Undisputed videos from Youtube
                    var undisputed = await getVideos(teamName, channelIDs.undisputed, "undisputed", youtubeAPIKeys[1]);
                    var undisputed_result = {name: undisputed_info.name, logo: undisputed_info.logo, content: undisputed.items};
                    ...
                    */
                    if (sport === "nba"){
                        ...
                        var nba = await getVideos(teamName, channelIDs.nba, "nba.com", youtubeAPIKeys[3]);
                        var nba_result = {name: nba_info.name, logo: nba_info.logo, content: nba.items};
                        results.push(nba_result);
                    }else{
                        // Request NFL.com videos from Youtube
                        var nfl = await getVideos(teamName, channelIDs.nfl, "nfl.com", youtubeAPIKeys[3]);
                        var nfl_result = {name: nfl_info.name, logo: nfl_info.logo, content: nfl.items};
                        results.push(nfl_result);
                    }
                    // Render the general video view, passing the sport, team and list of results params.
                    res.render("feed/general/video", {sport: req.params.sport, team: req.params.team, results:results});
                /*
    ...
    */
        ...
        */
        for (var i = 0; i < req.user.followed.length; i++){
            ...
            */
            try{
                results = await getAllVideos(req.user.followed[i].teamname, youtubeAPIKeys[0]);
            }catch{
                try{
                    results = await getAllVideos(req.user.followed[i].teamname, youtubeAPIKeys[1]);
                }catch{
                    try{
                        results = await getAllVideos(req.user.followed[i].teamname, youtubeAPIKeys[3]);
                    }catch{
...
function getArticles(source, domain, teamName, apikey){
    return new Promise((resolve, reject) => {
        request("https://newsapi.org/v2/everything?q="+teamName+
                ...
                "&apiKey=" + apikey, function(error, response, body){
                if(!error && response.statusCode == 200){
                    parsedBody = JSON.parse(body);
                    return resolve(parsedBody);
                }else{
